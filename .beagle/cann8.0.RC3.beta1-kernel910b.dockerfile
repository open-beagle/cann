...
RUN mkdir -p /tmp/Ascend-cann/ && \
  curl -fL -o /tmp/Ascend-cann/Ascend-cann-toolkit_8.0.RC3_linux-aarch64.run \
    ...
    chmod +x /tmp/Ascend-cann/Ascend-cann-toolkit_8.0.RC3_linux-aarch64.run && \
    /tmp/Ascend-cann/Ascend-cann-toolkit_8.0.RC3_linux-aarch64.run \
    --quiet --install --install-for-all --extract=/tmp/Ascend-cann/toolkit && \
  curl -fL -o /tmp/Ascend-cann-kernels-910b_8.0.RC3_linux-aarch64.run \
    ...
    chmod +x /tmp/Ascend-cann-kernels-910b_8.0.RC3_linux-aarch64.run && \
    /tmp/Ascend-cann-kernels-910b_8.0.RC3_linux-aarch64.run \
    --quiet --install --install-for-all --extract=/tmp/Ascend-cann/kernels-910b && \
